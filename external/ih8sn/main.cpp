...
std::map<std::string, std::string> load_config() {
    ...
    if (std::ifstream file("/system_ext/etc/ih8sn.conf"); file.good()) {
        std::string line;
...
int main(int argc, char *argv[]) {
    ...
    const auto first_api_level = config.find("FIRST_API_LEVEL");
    ...
    if (is_boot_completed_stage && first_api_level != config.end()) {
        property_override(property_list("ro.product.", "first_api_level"), first_api_level->second.c_str());
    }
